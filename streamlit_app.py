...
for i, cat_name in enumerate(categories):
    with all_tabs[i + 2]:
        ...
        for item in active:
            ...
            if c1.checkbox("Done", key=f"check_{item['id']}"):
                supabase.table("bucket_items").update({"is_completed": True}).eq("id", item['id']).execute()
                st.rerun()

            # Content
            c2.write(f"**{item['task_name']}**")
            if item['image_url']:
                c2.image(item['image_url'], width=150)

            # Favorite Toggle
            heart_label = "❤️" if item['is_favorite'] else "🤍"
            if c3.button(heart_label, key=f"fav_{item['id']}"):
                supabase.table("bucket_items").update({"is_favorite": not item['is_favorite']}).eq("id",
                                                                                                   item['id']).execute()
                st.rerun()
        ...
        with st.expander("✅ See Completed Items"):
            ...
            for item in done:
                if st.checkbox(f"~~{item['task_name']}~~", value=True, key=f"done_{item['id']}") == False:
